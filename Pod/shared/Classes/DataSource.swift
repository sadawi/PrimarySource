...
open class DataSource: NSObject {
    ...
    func item(atIndexPath indexPath: IndexPath) -> CollectionItemType? {
        return self[(indexPath as NSIndexPath).section]?.itemAtIndex((indexPath as NSIndexPath).row)
    }
    ...
    func canMoveItem(atIndexPath indexPath:IndexPath) -> Bool {
        ...
        return self.reorderingMode != .none && section.reorderable && section.itemAtIndex((indexPath as NSIndexPath).row)?.reorderable == true
    }
    ...
    func canMoveItem(fromIndexPath:IndexPath, toIndexPath:IndexPath) -> Bool {
        ...
        guard (toIndexPath as NSIndexPath).row < toSection.itemCount else { return false }
        guard toSection.reorderable && toSection[(toIndexPath as NSIndexPath).row]?.reorderable == true else { return false }
